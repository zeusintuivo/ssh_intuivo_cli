...
Script to port knocking
 call
     $0 __domain_or_ip_knock___    __domain_or_ip_enter__        ___from_file_path____  __to_remove_location__
     $0  website.com                web_key_name_config_file     /home/dev/file.py      /home/server/server/file.py
...
if [[ ! -e "${3}" ]] ; then 
{
	echo "error File ${3} cannot be found"
	exit 1
}
fi
